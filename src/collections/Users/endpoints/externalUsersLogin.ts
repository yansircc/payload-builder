    ...
    if (foundUser.totalDocs > 0) {
      try {
        ...
        if (loginAttempt?.token) {
          const usersCollection = (req.payload.collections as { [key: string]: Collection })[
            'users'
          ]
          if (!usersCollection) {
            throw new APIError('Users collection not found', 500, null, true)
          }
          const collection: Collection = usersCollection

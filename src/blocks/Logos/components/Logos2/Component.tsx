...
export default function Logos2({ title, description, logos, link }: Logos2Fields) {
  return (
        ...
        <div className="grid overflow-hidden rounded-xl border border-border md:grid-cols-2">
          <div className="my-auto px-6 py-10 sm:px-10 sm:py-12 lg:p-16">
            <div className="w-full md:max-w-md">
              <h2 className="mb-4 text-2xl font-semibold lg:text-3xl">{title}</h2>
          ...
          <div className="grid grid-cols-3 border-t border-border md:border-l md:border-t-0">
            {logos?.map((item, index) => (
              ...
              >
                <Media
                  resource={item.logo}
                  imgClassName="size-12 object-cover object-center sm:size-16 lg:size-24"
                  size="thumbnail"
                />
              </ClientMotionDiv>
            ))}

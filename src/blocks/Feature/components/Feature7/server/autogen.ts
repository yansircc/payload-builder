...
export async function autogen() {
  try {
    const { object } = await getObject(feature7Schema, USER_PROMPT, SYSTEM_PROMPT)
    return object
  } catch (error) {
    console.error('Error generating Feature 7 content:', error)
    throw error
  }
}
